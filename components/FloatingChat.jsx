...
export default function FloatingChat({
  engineType = 'excalidraw', // ✨ v6.0: 当前引擎类型
  ...
  // ✨ v6.0: 引擎选项
  const engineOptions = [
    // { value: 'drawio', label: 'Draw.io' }, 不在使用drawIO,后面在改
...
useEffect(() => {
  // Only reset when conversationId actually changes (not on initial mount or same value)
...
useEffect(() => {
  return () => {
    // Revoke all image URLs when component unmounts
    images.forEach(img => img?.url && URL.revokeObjectURL(img.url));
  };
}, []); // Only on unmount
