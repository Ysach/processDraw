...
export default function DrawPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved === 'drawio' || saved === 'excalidraw') {
        // 延迟到下一帧再设置，避免在 effect 中同步调用 setState 造成级联渲染
        Promise.resolve().then(() => setEngineType(saved));
      }
    } catch (e) {
  ...
  // 监听引擎类型变化，恢复待处理的历史记录
  useEffect(() => {
    // 当engineType改变且有pendingHistory时，恢复历史记录
    if (pendingHistory && engine) {
      engine.handleRestoreHistory(pendingHistory)
        .then(() => {
          // 恢复完成后清除pending状态
  ...
  }, [engineType, engine, pendingHistory]);
  ...
  useEffect(() => {
    ...
    if (savedConfig) {
      // 延迟到下一帧再设置，避免在 effect 中同步调用 setState 造成级联渲染
      Promise.resolve().then(() => setConfig(savedConfig));
    }

    // Load password access state
    const passwordEnabled = configService.isPasswordMode();
    Promise.resolve().then(() => setUsePassword(passwordEnabled));
